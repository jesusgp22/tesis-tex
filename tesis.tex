...
\begin{algorithm}[H]
	\caption{Calculo de horas activas}\label{alg:sync}
	\begin{algorithmic}[1]
		...
		\State $\textit{start\_date} \gets \textit{parameters.end\_date} $
...
A diferencia de las horas activas, definimos a un día como "activo" cuando podemos recoger un periodo de actividad continua de al menos una hora durante ese día, como se explicó en la Subsección \ref{sub:disponibilidad} 

\subsubsection{Cálculo de periodos de actividad continúa}

\subsubsection{Cálculo de mapas geo-ip}
...
Through the development of this work we built both a lightweight but powerful network monitor capable of monitoring links in arbitrary network setups as well as a web platform to manage, gather data and analyze the results from these monitors.
