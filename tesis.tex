...
Los resultados de las pruebas generalmente consisten de una marca de tiempo, un enlace relacionado y uno o mas valores conteniendo los datos obtenidos de las pruebas, desafortunadamente no hay forma de guardar datos de esta manera "semi-estructurada" en bases de datos estructuradas por lo que sería necesario definir dos tablas, una para guardar la marca de tiempo de la traza y el tipo de prueba y otra para guardar la lista de datos relacionados a esa prueba, sin embargo esto sería prohibitivamente ineficiente ya que habría que hacer varios joins en tablas de potencialmente millones de entradas solo para obtener los datos relacionados a una traza, igualmente se podría estar tentado a usar una tabla con el timestamp y un campo text para guardar datos en algún formato (como json), pero de nuevo esto sería ineficiente en términos de espacio de almacenamiento y no se podrían aprovechar las características del manejador de bases de datos para calcular promedios, máximos, mínimos o filtrar trazas por columnas específicas.



The datastore class is necessary to store the test results (or traces) collected from the monitoring, while a single generic table with a timestamp, related link identifier and a json field to store unstructured data would suffice, this would be wasteful  and make filtering and calculating averages prohibitively inefficient, for this reason a custom-made datastore class (that will be later mapped to a database table by Django ORM) should be created to fit the needs of each specific test.

Django includes a powerful ORM that can help building and maintain any arbitrary sql database structure, changes done to models are tracked by migration files that can be created on the development environment and quickly applied to the production environment with a migration command. 



\section{Cargador de visualizaciones}
