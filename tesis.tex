...
Existen dos paradigmas fundamentales en el monitoreo de redes, el monitoreo activo que consiste en generar tráfico y observar la respuesta de la red y el monitoreo pasivo, que consiste en observar el tráfico que atraviesa un cierto enlace o nodo. Para tener una imagen completa de la red es conveniente usar ambos paradigmas ya que ambos pueden ofrecer distintas perspectivas del estado de la red.

Algunos casos de uso típicos del monitoreo de redes incluyen:

\begin{itemize}
	\item{Sistemas de detección de intrusos: los sistemas de detección de intrusos (IDS) intentan detectar la presencia de activades maliciosas en una red }
	\item{Tomografía de redes: consiste en obtener datos sobre el estado de ciertos enlaces a partir de sondas enviadas por nodos en los puntos finales de una red. Este es un caso muy común ya que en el caso de Internet rara vez se tiene acceso a los nodos internos manejados por ISPs}
	\item{Análisis de rutas: es el conjunto de métodos y herramientas para obtener datos sobre el enrutamiento de paquetes, el enrutamiento es vital en el funcionamiento de redes ip y problemas como ineficiencias o ciclos pueden aparecer y degradar el servicio}
