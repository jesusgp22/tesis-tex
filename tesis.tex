...
Debido a la rápida evolución en la capacidad de almacenamiento de los sistemas computaciones y la adopción por parte de miles de millones de usuarios de, así como el despliegue de millones de sensores y objetos inteligentes (IoT) se están generando datos a una velocidad vertiginosa y creciente, está claro que es necesaria la existencia de sistemas y métodos para obtener información útil a partir de este flujo masivo de datos. 

Big Data se refiere la tendencia reciente de recolectar, almacenar y hacer análisis sobre cantidades masivas de datos, no existe una cantidad precisa a partir de la cual una aplicación pueda considerarse de "big data", pero usualmente el termino se relaciona con datos en el orden de los petabytes (10\textsuperscript{15} bytes) y exabytes (10\textsuperscript{18} bytes) \cite{barranco2012}.

 permitiendo a organizaciones tomar decisiones basadas en el análisis de estos datos (tambien llamadas data-driven desitions )
...
La visualización de datos se refiere al conjunto de elementos gráficos y técnicas usadas para representar la información cuantitativa; cualquier conjunto de datos no tienen significado sin alguna manera de organizar y  presentar los descubrimientos relevantes que se encuentran potencialmente ocultos dentro de estos.
...
Es importante elegir los métodos apropiados para visualizar distintos conjuntos de datos, por ejemplo, los datos discretos se pueden observar a través de gráficas de barras, los gráficos de redes pueden comunicar efectivamente la relación entre distintos entes, los mapas son efectivos para desplegar información geográfica, los mapas de calor permiten comparar el rendimiento de una variable a través del tiempo, etc.

Existe una gran riqueza de métodos para comunicar la información de forma creativa, el uso de colores puede ser aprovechado para ofrecer intuitivamente información sobre grupos y categorías, es posible combinar distintos conceptos para lograr visualizaciones aun mas poderosas como mapas de calor superpuestos en mapas pueden expresar la "densidad" en un área.
...
Las técnicas mas comunes para lograr esto es tener elementos que ocupen el mayor espacio horizontal posible en pantallas grandes y mientras el tamaño horizontal se reduce estos pasan a ocupar el espacio verticalmente; siempre buscamos ocupar solo el máximo del ancho de la pantalla, evitando crear barras de desplazamiento horizontal que desorientan e incomodan a los usuarios. 
...
El diseño web adaptable no solo facilita el desarrollo de sitios web ahorrando a los desarrolladores y diseñadores construir múltiples versiones de un mismo sitio web sino que ademas el sitio web ofrece una experiencia similar independientemente del dispositivo con el que se este visitando.
